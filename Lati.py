...
# ----------------- 2. DASHBOARD GALMEE BARATTOOTAA (FOOMII) -----------------
  ...
  with st.form("registration_form"):
    ...
    with col1:
      ...
      b_guyyaa = b_col1.selectbox("Guyyaa", [str(i) for i in range(1, 31)])
...
# ----------------- 3. DASHBOARD BARSIIKAA / GABAASAA -----------------
  ...
  if password == "kitesa2019" or password == "admin123":
    ...
    with tabH:
      st.markdown("### H. Gabaasa Lakkoofsaa Barattoota Irra Deebi'anii")
      if not db.empty:
        repeat_df = db[db["Haala Galmee"].str.contains("Irra deebii", na=False)]
        st.dataframe(
    ...
    with tabI:
      st.markdown("### I. Gabaasa Galmee Waligalaa Bara 2019")
      perf_data = []
      for k in range(1, 13):
        k_str = str(k)
    ...
    with tabJ:
      ...
      if not db.empty:
        idx_to_modify = st.number_input(
            "Lakk. Index Barataa jijjiiruu/haquu barbaadde",
            min_value=0,
